<h2 class="sub-header">Użytkownicy (<?= $this->allUsers ?>)</h2>
<div class="table-responsive">
    <script>
        function deleteConfirm(deleteLink){
            var r = confirm("Czy na pewno chcesz usunąć tego użytkownika ?");
            if (r == true) {
        ...
        <tbody>  
            ...
            foreach ($this->users as $user) {
                echo '<tr>
                            ...
                            <td><a href="' . URL . 'devicesettings/userdevices/' . $user['id'] . '">' . $user['deviceCount'] . '</a></td>
                            <td>' . @date("d-m-Y", $user['datarejestracji']) . '</td>
                            <td>' . $user['stanOpis'] . '</td>
                            <td>' . $user['oplataOpis'] . '</td>
                ...
                echo '<button type="submit" class="btn btn-danger" onclick="deleteConfirm( \'' . URL . "userchange/delete/" . $user['id'] . "/usersettings/" .
                        $this->showUsersCallbackLink . '/' . $this->listing['count'] . '/' . $this->listing['orderBy'] . '/' . $this->listing['oldsort'] . '/' . $this->activePage . '\')">Usuń</button>
                        </div></td>
                      </tr>';
                
               // echo '<button type="submit" class="btn btn-danger" onclick="window.location.href = \'' . URL . "userchange/delete/" . $user['id'] . "/usersettings/" .
                //$this->showUsersCallbackLink . '/' . $this->listing['count'] . '/' . $this->listing['orderBy'] . '/' . $this->listing['oldsort'] . '/' . $this->activePage . '\'">Usuń</button>
